...
trait x535 extends RootController {
  ...
  io.memStreams.stores(0).cmd.bits.addr := Utils.getRetimed(x512(63,0), 0)
  io.memStreams.stores(0).cmd.bits.size := Utils.getRetimed(x512(95,64), 0)
  io.memStreams.stores(0).cmd.valid :=  x512_valid.D(0, rr)
  io.memStreams.stores(0).cmd.bits.isWr := Utils.getRetimed(~x512(96,96), 0)
